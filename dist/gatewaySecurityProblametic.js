...
function validateGatewayHeaders(req) {
  // 1️⃣ HARD TRUST CHECK – MUST BE FIRST
  if (req.headers["x-jwt-verified"] !== "true") {
    return {
      valid: false,
...
// }
function isTokenExpired(req) {
  const rawHeader = req.headers["x-token-expires-at"];

  console.log("DEBUG expiry header:", rawHeader);
  console.log("DEBUG now ms:", Date.now());

  if (!rawHeader) {
    console.warn("Missing x-token-expires-at header");
    return false;
  }

  const expirySeconds = Number(rawHeader);

  if (!Number.isFinite(expirySeconds)) {
    console.warn("Invalid expiry header:", rawHeader);
    return false;
  }
  ...
  console.log("DEBUG expiry ms:", expiryMs);
  console.log("DEBUG expiry + grace:", expiryMs + graceMs);

  return now > expiryMs + graceMs;
}
...
function validateGatewayRequest(req) {
  ...
  //   const expiresAt = req.headers["x-token-expires-at"];
  //   const expirySeconds = expiresAt ? Number(expiresAt) : null;

  //   // Convert JWT exp (seconds) → milliseconds
  //   const expiryTimeMs = Number.isFinite(expirySeconds)
  //     ? expirySeconds * 1000
  //     : null;

  //   const now = Date.now();

  //   const expiredBy = expiryTimeMs !== null ? now - expiryTimeMs : null;

  //   // const gracePeriodMs = parseInt(
  //   //   process.env.TOKEN_EXPIRY_GRACE_PERIOD_MS || "60000",
  //   //   10
  //   // );
  //   if (expiryTimeMs !== null && now > expiryTimeMs + gracePeriodMs) {
  //     // token is expired
  //   }
